...
static void printf(char* format, ...)
{
	...
	for (size_t n = 0; format[n]; n++)
	{
		if (format[n] == '%')
		{
			...
			case 'x':
				unsigned argi;
				argi = va_arg(valist, unsigned);
				kputhex_basic(argi);
				break;
			default:
				kputs_basic("%?");
				kputc_basic(format[n + 1]);
				break;
			}
			n++;
		}
...
static int l_pageSize  = PAGESIZE;			//< Individual page size
static int l_pageCount = 16;			//< Minimum number of pages to allocate.

//ChaiOS Interface
static CSpinlock lalloc_lock;
int liballoc_lock()
{
	lalloc_lock.acquireSpinlock();
	return 0;
}

int liballoc_unlock()
{
	lalloc_lock.releaseSpinlock();
	return 0;
}

void* liballoc_alloc(int pages)
{
	const CVMemMngr::PAGING_ATTRIBUTES& attr = getMemoryManager()->getVMemMngr()->getDefaultAttributes(true);
	virtaddr allocpages = getMemoryManager()->getVMemMngr()->allocate(pages*PAGESIZE, attr);
	return allocpages;
}

int liballoc_free(void* addr, int pages)
{
	getMemoryManager()->getVMemMngr()->free(addr, pages*PAGESIZE);
	return 0;
}
...
CHAIOS_API_FUNC(void* DECL_CALL malloc(size_t size))
{
	int index;
	void *ptr;
	struct boundary_tag *tag = NULL;
...
CHAIOS_API_FUNC(void DECL_CALL free(void *ptr))
{
	int index;
	struct boundary_tag *tag;
...
CHAIOS_API_FUNC(void* DECL_CALL calloc(size_t nobj, size_t size))
{
       int real_size;
       void *p;
...
CHAIOS_API_FUNC(void* DECL_CALL realloc(void *p, size_t size))
{
	void *ptr;
	struct boundary_tag *tag;
	int real_size;
