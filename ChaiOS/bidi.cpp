...
#define MAX_CCH 256
void ShowInputTypes(LPTSTR pszInput, int cch)
{
...
bool isRtl(const wchar_t* inputString)
{
	...
	classify(inputString, types, cch);
	...
	return baselevel == 1 ? false : true;
}

//Runs the BIDI algorithm on the input string.
wchar_t* BidiInterface(const wchar_t* inputString)
{
	...
	classify(inputString, types, cch, true);

	TCHAR* pszInput = new TCHAR[MAX_CCH+1];

	memcpy(pszInput, inputString, (cch+1)*sizeof(TCHAR));
